...
#ifdef RT_USING_COMMON_TEST_LINUX_TTY_RPMSG_LITE

rpmsg_queue_handle g_remote_queue;
uint32_t g_master_ept_id;
struct rpmsg_info_t *g_info;

static void rpmsg_tty_recv_thread(void *parameter)
{
    char *rx_msg = (char *)rt_malloc(RL_BUFFER_PAYLOAD_SIZE);

    if (!rx_msg)
    {
        rt_kprintf("rx_msg malloc failed\n");
        return;
    }

    while (1)
    {
        rpmsg_queue_recv(g_info->instance, g_remote_queue, (uint32_t *)&g_master_ept_id, rx_msg, RL_BUFFER_PAYLOAD_SIZE, RL_NULL, RL_BLOCK);
        rt_kprintf("tty rpmsg remote: master_ept_id-0x%lx rx_msg: %s\n", g_master_ept_id, rx_msg);
    }
}

static void rpmsg_tty_test(void)
{
    uint32_t master_id, remote_id;
    struct rpmsg_block_t *block;
    uint32_t ept_flags;
    ...
    g_info = malloc(sizeof(struct rpmsg_info_t));
    if (g_info == NULL)
    {
        rt_kprintf("g_info malloc error!\n");
        return;
    }

    g_info->private = malloc(sizeof(rpmsg_queue_handle)); // 用于传递 queue
    if (g_info->private == NULL)
    {
        rt_kprintf("private malloc error!\n");
        return;
    }

    rt_kprintf("tty rpmsg remote: shmem_base-0x%lx shmem_end-%lx\n", RPMSG_LINUX_MEM_BASE, RPMSG_LINUX_MEM_END);
    g_info->instance = rpmsg_lite_remote_init((void *)RPMSG_LINUX_MEM_BASE, RL_PLATFORM_SET_LINK_ID(master_id, remote_id), RL_NO_FLAGS);
    rpmsg_lite_wait_for_link_up(g_info->instance, 10U);
    rt_kprintf("tty rpmsg remote: link up! link_id-0x%lx\n", g_info->instance->link_id);

    rpmsg_ns_bind(g_info->instance, rpmsg_ns_cb, &ns_cb_data);
    g_remote_queue  = rpmsg_queue_create(g_info->instance);
    g_info->ept = rpmsg_lite_create_ept(g_info->instance, RPMSG_RTT_REMOTE_TEST3_EPT_ID, rpmsg_queue_rx_cb, g_remote_queue);
    ept_flags = RL_NS_CREATE;
    rpmsg_ns_announce(g_info->instance, g_info->ept, "rpmsg-tty", ept_flags);

    rt_kprintf("tty rpmsg init success\n");

    // Create a receiving thread
    rt_thread_t tid = rt_thread_create("rpmsg_rx",
                                        rpmsg_tty_recv_thread,
                                        RT_NULL,
                                        1024,
    ...
    if (tid != RT_NULL)
        ...
        rt_kprintf("create rpmsg recv thread failed!\n");

}

INIT_APP_EXPORT(rpmsg_tty_test);

static void rpmsg_tty_send(int argc, char **argv)
{
    if (argc != 2)
    {
        rt_kprintf("Usage: rpmsg_send <message>\n");
        return;
    }

    if (g_info == NULL || g_master_ept_id == 0)
    {
        rt_kprintf("RPMsg not ready or no master_ept_id available\n");
        return;
    }

    const char *msg = argv[1];
    int len = rt_strlen(msg);
    if (len > 64)
    {
        rt_kprintf("Message too long (max %d bytes)\n", 64);
        return;
    }

    int ret = rpmsg_lite_send(g_info->instance, g_info->ept,
                              g_master_ept_id, msg, len, RL_BLOCK);
    if (ret < 0)
        rt_kprintf("rpmsg_lite_send failed: %d\n", ret);
}

MSH_CMD_EXPORT(rpmsg_tty_send, rpmsg_send msg to linux tty);
...
#endif  //RT_USING_COMMON_TEST_LINUX_TTY_RPMSG_LITE
